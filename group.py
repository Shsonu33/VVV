...
RAHUL_PATH = "./Rahul"  # Path to Rahul executable
...
def handle_attack(message):
    ...
    def update_timer():
        while time.time() < attack_end_time:
            remaining_time = int(attack_end_time - time.time())
            bot.send_message(message.chat.id, f"⏳ **Attack in Progress:** `{remaining_time}` seconds left!", parse_mode="Markdown")
            time.sleep(10)  # Update every 10 seconds
        
        bot.send_message(message.chat.id, "✅ **Attack finished!** Another user can attack now.")
